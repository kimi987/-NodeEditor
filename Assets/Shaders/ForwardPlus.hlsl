...
StructuredBuffer<LightIndex> _LightIndexes;
...
void GetAdditionalLight(in int loopIndex, in int buff, in float3 worldPosition,
    out Light light)
{
    uint lightIndex = (buff >> loopIndex) -(buff >> (loopIndex + 4) << 4) ;
    ForwardLight l = _Lights[lightIndex];

    float3 lightVector = l.m_worldPos - worldPosition;
    ...
    light.distanceAttenuation = l.m_lightAttenuation * max(0, (l.m_range * l.m_range - distanceSqr));
    ...
    in half3 emission, half alpha, in float4 clipPosition)
{
    ...
    uint2 index = floor(texcoord * numFrustums);
    LightIndex lightIndex = _LightIndexes[index.x + index.y * numFrustums.x];
    uint n = lightIndex.buff - (lightIndex.buff >> 4 << 4);
    Light light;
    for (int i = 1; i <= n; ++i)
    {
        GetAdditionalLight(i * 4, lightIndex.buff, inputData.positionWS, light);
